...
exports.initializeOIDCFromDB = initializeOIDCFromDB;
exports.updateOIDCConfigInDB = updateOIDCConfigInDB;
exports.getOIDCConfig = getOIDCConfig;
exports.initializeOIDCMiddleware = initializeOIDCMiddleware;
...
async function initializeOIDCFromEnv() {
    try {
        ...
        if (process.env.OIDC_ISSUER_BASE_URL &&
            ...
            logger_1.logger.info('Initializing OIDC from environment variables');
            oidcConfig = {
                ...oidcConfig,
                issuerBaseURL: process.env.OIDC_ISSUER_BASE_URL,
                clientID: process.env.OIDC_CLIENT_ID,
        ...
        logger_1.logger.error('Error initializing OIDC from environment:', error);
        return false;
    }
}
/**
 ...
 */
async function initializeOIDCFromDB() {
    try {
        ...
        if (!config || !config.enabled) {
            logger_1.logger.info('OIDC is not enabled or configured in database');
            return false;
        }
        if (!config.clientId || !config.clientSecret || !config.discoveryUrl) {
            logger_1.logger.error('OIDC client ID, secret, or discovery URL is missing');
            return false;
        }
        // Update the config
        oidcConfig = {
        ...
        logger_1.logger.info('OIDC initialized from database configuration');
        return true;
    }
    catch (error) {
        logger_1.logger.error('Error initializing OIDC from database:', error);
        return false;
    }
}
/**
 ...
 */
async function updateOIDCConfigInDB(configData) {
    try {
 ...
 */
function initializeOIDCMiddleware(app) {
    // Check if we have a valid configuration before initializing
    if (!oidcConfig.issuerBaseURL) {
        logger_1.logger.warn('OIDC middleware not initialized: issuerBaseURL is empty');
        return;
    }
    if (!oidcConfig.clientID) {
        logger_1.logger.warn('OIDC middleware not initialized: clientID is empty');
        return;
    }
    if (!oidcConfig.clientSecret) {
        logger_1.logger.warn('OIDC middleware not initialized: clientSecret is empty');
        return;
    }
    // Initialize OIDC middleware with current config
    app.use((0, express_openid_connect_1.auth)(oidcConfig));
    logger_1.logger.info('OIDC middleware initialized with current configuration');
}
/**
 * Initialize OIDC
